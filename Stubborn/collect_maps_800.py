...
def main():
    ...
    config_paths = 'Stubborn/collect_config.yaml'
    ...
    num_episodes = 800 * config.ENVIRONMENT.ITERATOR_OPTIONS.MAX_SCENE_REPEAT_EPISODES
    ...
    while count_episodes < min(num_episodes, end):
        ...
        if count_episodes >= start and count_episodes < end:
            ...
            if np.sum(full_map_seq[:, 4:]) > 0 and np.sum(full_map_seq[:, 1]) > 4000:
                np.savez_compressed('./data/saved_maps/%s_full/f%05d.npz' % (config.DATASET.SPLIT, count_episodes), maps=full_map_seq)
